...
//    echo $gateway;
    ...
    $query = $conn->prepare('SELECT gateway_id FROM `gateway` WHERE name=:name');
    $query->execute(array(
        ':name' => $gateway
    ));
    ...
    //TODO: ROWCOUNT
    $sql = ("UPDATE gateway SET name='$gateway', longitude='$long', latitude='$lat', description='$desc' WHERE gateway_id='$gatewayId'");
    echo $sql;

    $sql = $conn->prepare("UPDATE gateway SET name=:name, longitude=:longitude, latitude=:latitude, description=:description WHERE gateway_id=:id");
    if (!$sql){
        echo "\nPDO::errorInfo():\n";
